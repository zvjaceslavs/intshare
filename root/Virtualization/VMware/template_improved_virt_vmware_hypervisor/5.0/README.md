...
|Average read latency of the datastore $3|<p>-</p>|`Simple check`|vmware.hv.datastore.read[{$URL},{HOST.HOST},{#DATASTORE},latency]<p>Update: 300</p>|
|Average write latency of the datastore $3|<p>-</p>|`Simple check`|vmware.hv.datastore.write[{$URL},{HOST.HOST},{#DATASTORE},latency]<p>Update: 300</p>|
|Free space on datastore $3|<p>-</p>|`Simple check`|vmware.hv.datastore.size[{$URL},{HOST.HOST},{#DATASTORE},free]<p>Update: 300</p>|
|Free space on datastore $3 (percentage)|<p>-</p>|`Simple check`|vmware.hv.datastore.size[{$URL},{HOST.HOST},{#DATASTORE},pfree]<p>Update: 300</p>|
|Total size of datastore $3|<p>-</p>|`Simple check`|vmware.hv.datastore.size[{$URL},{HOST.HOST},{#DATASTORE}]<p>Update: 300</p>|
## Triggers

|Name|Description|Priority|
|----|-----------|----|
|{HOST.NAME} - CPU Usage (5 min avg) is {ITEM.LASTVALUE}|<p>-</p>|warning|
|{HOST.NAME} - CPU Usage (5 min avg) is {ITEM.LASTVALUE}|<p>-</p>|average|
|{HOST.NAME} - ESXi Health State Is {ITEM.LASTVALUE}|<p>The host health state rollup sensor value: gray - unknown, green - okay, red - it has a problem, yellow - it might have a problem.</p>|high|
|{HOST.NAME} - ESXi Overall Status Is {ITEM.LASTVALUE}|<p>The host overall status sensor: gray - unknown, green - okay, red - it has a problem, yellow - it might have a problem.</p>|high|
|{HOST.NAME} - Memory usage (5 min avg) is {ITEM.LASTVALUE1} ({ITEM.LASTVALUE1})|<p>-</p>|average|
|{HOST.NAME} - Memory usage (5 min avg) is {ITEM.LASTVALUE1} ({ITEM.LASTVALUE2})|<p>-</p>|warning|
|{HOST.NAME} - Free space on datastore {#DATASTORE} is {ITEM.LASTVALUE1} - {ITEM.LASTVALUE2} remaining|<p>-</p>|average|
|{HOST.NAME} - Free space on datastore {#DATASTORE} is {ITEM.LASTVALUE1} - {ITEM.LASTVALUE2} remaining|<p>-</p>|high|
