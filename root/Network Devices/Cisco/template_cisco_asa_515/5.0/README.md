...
|{#ENT_NAME}: Hardware serial number|<p>MIB: ENTITY-MIB</p>|`SNMP agent`|system.hw.serialnumber[entPhysicalSerialNum.{#SNMPINDEX}]<p>Update: 30s</p>|
|{#SENSOR_INFO}: Fan status|<p>MIB: CISCO-ENVMON-MIB</p>|`SNMP agent`|sensor.fan.status[ciscoEnvMonFanState.{#SNMPINDEX}]<p>Update: 30s</p>|
|$1 AdminStatus|<p>The desired state of the interface.</p>|`SNMP agent`|ifAdminStatus[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 Description|<p>A textual string containing information about the interface. This string should include the name of the manufacturer, the product name and the version of the interface hardware/software.</p>|`SNMP agent`|ifDescr[{#SNMPVALUE}]<p>Update: 3600</p>|
|$1 InBroadcastPkts|<p>-</p>|`SNMP agent`|ifInBroadcastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
...
|$1 InMulticastPkts|<p>-</p>|`SNMP agent`|ifInMulticastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 InOctets|<p>The number of octets in valid MAC frames received on this interface, including the MAC header and FCS.</p>|`SNMP agent`|ifInOctets[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 InUcastPkts|<p>-</p>|`SNMP agent`|ifInUcastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 OperStatus|<p>The current operational state of the interface.</p>|`SNMP agent`|ifOperStatus[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 OutBroadcastPkts|<p>-</p>|`SNMP agent`|ifOutBroadcastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
...
|$1 OutMulticastPkts|<p>-</p>|`SNMP agent`|ifOutMulticastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 OutOctets|<p>The number of octets transmitted in MAC frames on this interface, including the MAC header and FCS.</p>|`SNMP agent`|ifOutOctets[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 OutUcastPkts|<p>-</p>|`SNMP agent`|ifOutUcastPkts[{#SNMPVALUE}]<p>Update: 60</p>|
|$1 Speed|<p>-</p>|`SNMP agent`|ifSpeed[{#SNMPVALUE}]<p>Update: 60</p>|
...
|{#SENSOR_INFO}: Power supply status|<p>MIB: CISCO-ENVMON-MIB</p>|`SNMP agent`|sensor.psu.status[ciscoEnvMonSupplyState.{#SNMPINDEX}]<p>Update: 30s</p>|
|{#SNMPVALUE}: Temperature status|<p>MIB: CISCO-ENVMON-MIB The current state of the test point being instrumented.</p>|`SNMP agent`|sensor.temp.status[ciscoEnvMonTemperatureState.{#SNMPINDEX}]<p>Update: 180s</p>|
|{#SNMPVALUE}: Temperature|<p>MIB: CISCO-ENVMON-MIB The current measurement of the test point being instrumented.</p>|`SNMP agent`|sensor.temp.value[ciscoEnvMonTemperatureValue.{#SNMPINDEX}]<p>Update: 180s</p>|
## Triggers
...
|High ASA CPU utilization|<p>Last value: {ITEM.LASTVALUE1}.</p>|high|
|High CPU utilization|<p>Last value: {ITEM.LASTVALUE1}.</p>|high|
|High memory utilization|<p>Last value: {ITEM.LASTVALUE1}.</p>|average|
|IOS version has changed|<p>-</p>|information|
|Link Down on Inside Interface - DMZ|<p>Interface WAN</p>|disaster|
|Link Down on Inside Interface - DMZ2|<p>Interface WAN</p>|disaster|
|Link Down on Inside Interface - MGMT|<p>Interface de Gerenciamento</p>|disaster|
|Link Down on Inside Interface - WAN|<p>Interface WAN</p>|disaster|
|Link Down on LAN Interface|<p>Interface LAN</p>|disaster|
...
|Pacotes Entrada/Saida|<p>-</p>|information|
|Ping loss is too high on {HOST.NAME}|<p>Packet loss more than 20% on {HOST.IP}</p>|average|
|Port asa_mgmt_plane Interface|<p>-</p>|disaster|
|Port DMZ2 Interface|<p>-</p>|disaster|
|Port DMZ Interface|<p>-</p>|disaster|
|Port LAN Interface|<p>-</p>|disaster|
|Port WAN Interface|<p>-</p>|disaster|
|Response time is too high on {HOST.NAME}|<p>Response time more than 200ms for 10 minutes.</p>|warning|
|{HOST.NAME}  CPU 1 min|<p>{HOST.NAME} has had a CPU Utilization above 85 % for 3 samples taken in the last 1 minute 30 seconds.</p>|high|
|{HOST.NAME}  CPU 5 min|<p>{HOST.NAME} has had a CPU Utilization above 85 % for 3 samples taken in the last 1 minute 30 seconds.</p>|high|
|{HOST.NAME}  CPU Maior que 85 %|<p>{HOST.NAME} has had a CPU Utilization above 85 % for 3 samples taken in the last 1 minute 30 seconds.</p>|disaster|
|{HOST.NAME}  CPU Over 85 %|<p>{HOST.NAME} has had a CPU Utilization above 85 % for 3 samples taken in the last 1 minute 30 seconds.</p>|high|
|{HOST.NAME}  CPU Over 85 %|<p>{HOST.NAME} has had a CPU Utilization above 85 % for 3 samples taken in the last 1 minute 30 seconds.</p>|high|
|{HOST.NAME} is unavailable by ICMP|<p>{HOST.IP} not responding to ICMP in 1 minutes</p>|disaster|
|{HOST.NAME} Memory Free < 5 %|<p>Memory Free < 5 %</p>|disaster|
|{HOST.NAME} reloaded|<p>{HOST.NAME} reloaded</p>|high|
|{HOST.NAME} SNMP Failed|<p>{HOST.IP} SNMP Failed. Check configuration, ACL.</p>|high|
|{HOSTNAME} is Unreachable|<p>-</p>|disaster|
