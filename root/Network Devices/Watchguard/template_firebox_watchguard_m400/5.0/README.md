...
|Incoming traffic on interface $1|<p>The number of octets in valid MAC frames received on this interface in 1 second, including the MAC header and FCS.</p>|`SNMP agent`|ifInOctets[{#SNMPVALUE}]<p>Update: 30s</p>|
|Operational status of interface $1|<p>The current operational state of the interface.</p>|`SNMP agent`|ifOperStatus[{#SNMPVALUE}]<p>Update: 1m</p>|
...
|Outgoing traffic on interface $1|<p>The number of octets transmitted in MAC frames on this interface per second , including the MAC header and FCS.</p>|`SNMP agent`|ifOutOctets[{#SNMPVALUE}]<p>Update: 1m</p>|
|Traffic on {#SNMPVALUE}|<p>-</p>|`SNMP agent`|Traffic[{#SNMPVALUE}]<p>Update: 1m</p>|
|Processor $1 type|<p>Processor type</p>|`SNMP agent`|hrDeviceDescr[{#SNMPINDEX}]<p>Update: 1m</p>|
|Utilization of processor $1|<p>The average, over the last minute, of the percentage of time that this processor was not idle. Implementations may approximate this one minute smoothing period if necessary.</p>|`SNMP agent`|hrProcessorLoad[{#SNMPINDEX}]<p>Update: 1m</p>|
|Description of storage $1|<p>A description of the type and instance of the storage described by this entry.</p>|`SNMP agent`|hrStorageDescr[{#SNMPVALUE}]<p>Update: 30s</p>|
|Total disk space on $1|<p>total disk space in bytes.</p>|`SNMP agent`|hrStorageSize[{#SNMPVALUE}]<p>Update: 30s</p>|
|Used disk space on $1|<p>Used disk space in bytes.</p>|`SNMP agent`|hrStorageUsed[{#SNMPVALUE}]<p>Update: 30s</p>|
## Triggers

|Name|Description|Priority|
|----|-----------|----|
|Lack of available memory on {HOST.NAME}|<p>-</p>|average|
|Operational status was changed on {HOST.NAME} interface {#SNMPVALUE}|<p>-</p>|information|
|Utilization of processor {#SNMPINDEX} over 70%|<p>CPU on {HOST.NAME} is over 70%</p>|average|
