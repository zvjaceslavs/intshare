...
|Rotation speed of $1|<p>-</p>|`SNMP agent`|fanValue[{#SNMPVALUE}]<p>Update: 300</p>|
|Disk size of $1|<p>Capacity of disk</p>|`SNMP agent`|diskSize[{#SNMPVALUE}]<p>Update: 3600</p>|
|Disk status of $1|<p>Status of Disk. There are four status: NORMAL FOREIGN FAULTED MISSING</p>|`SNMP agent`|diskStatus[{#SNMPVALUE}]<p>Update: 600</p>|
|IO average queue length of $1|<p>Average queue length.</p>|`SNMP agent`|ioAvgQueue[{#SNMPVALUE}]<p>Update: 300</p>|
|IO average service time at disk of $1|<p>Average service time at disk.</p>|`SNMP agent`|ioAvgSvc[{#SNMPVALUE}]<p>Update: 300</p>|
|IO average disk utilization of $1|<p>Average disk utilization.</p>|`SNMP agent`|ioAvgUtil[{#SNMPVALUE}]<p>Update: 300</p>|
|IO average waiting time of $1|<p>Average waiting time (queue + service)</p>|`SNMP agent`|ioAvgWait[{#SNMPVALUE}]<p>Update: 300</p>|
|IO CPU time (%) of $1|<p>Percentage of CPU time during which I/O requests were issued to the device.</p>|`SNMP agent`|ioCpuTime[{#SNMPVALUE}]<p>Update: 300</p>|
|IO kilobytes read per second of $1|<p>Kilobytes read per second.</p>|`SNMP agent`|ioKbrs[{#SNMPVALUE}]<p>Update: 300</p>|
|IO kilobytes write per second of $1|<p>Kilobytes written per second.</p>|`SNMP agent`|ioKbws[{#SNMPVALUE}]<p>Update: 300</p>|
|IO read merges per second of $1|<p>Read merges per second.</p>|`SNMP agent`|ioMgrs[{#SNMPVALUE}]<p>Update: 300</p>|
|IO write merges per second of $1|<p>Write merges per second.</p>|`SNMP agent`|ioMgws[{#SNMPVALUE}]<p>Update: 300</p>|
|IO read requests per second of $1|<p>Read io requests per second.</p>|`SNMP agent`|ioReqrs[{#SNMPVALUE}]<p>Update: 300</p>|
|IO write requests per second of $1|<p>Write io requests per second.</p>|`SNMP agent`|ioReqws[{#SNMPVALUE}]<p>Update: 300</p>|
|Connected client count of protocol $1|<p>-</p>|`SNMP agent`|conCount[{#SNMPVALUE}]<p>Update: 600</p>|
|Temperature of $1|<p>-</p>|`SNMP agent`|tempValue[{#SNMPVALUE}]<p>Update: 300</p>|
|Description of interface $1|<p>A textual string containing information about the interface. This string should include the name of the manufacturer, the product name and the version of the interface hardware/software.</p>|`SNMP agent`|ifDescr[{#SNMPVALUE}]<p>Update: 86400</p>|
...
|Incoming traffic on interface $1|<p>The number of octets in valid MAC frames received on this interface, including the MAC header and FCS.</p>|`SNMP agent`|ifInOctets[{#SNMPVALUE}]<p>Update: 120</p>|
|Operational status of interface $1|<p>The current operational state of the interface.</p>|`SNMP agent`|ifOperStatus[{#SNMPVALUE}]<p>Update: 120</p>|
...
|Outgoing traffic on interface $1|<p>The number of octets transmitted in MAC frames on this interface, including the MAC header and FCS.</p>|`SNMP agent`|ifOutOctets[{#SNMPVALUE}]<p>Update: 120</p>|
...
|Voltage of $1|<p>The voltage in V</p>|`SNMP agent`|voltValue[{#SNMPVALUE}]<p>Update: 300</p>|
...
|Storage total size in Bytes on $1|<p>The real size of the storage in Bytes. hrStorageSize * hrStorageAllocationUnits.</p>|`Calculated`|hrStorageSizeInBytes[{#SNMPVALUE}]<p>Update: 60</p>|
...
|Storage used in Bytes on $1|<p>The storage used in Bytes. hrStorageUsed * hrStorageAllocationUnits.</p>|`Calculated`|hrStorageUsedInBytes[{#SNMPVALUE}]<p>Update: 60</p>|
|Storage used on $1|<p>The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits.</p>|`SNMP agent`|hrStorageUsed[{#SNMPVALUE}]<p>Update: 60</p>|
## Triggers

|Name|Description|Priority|
|----|-----------|----|
|Media service status was changed for: {ITEM.LASTVALUE}|<p>-</p>|information|
|RAID Level was changed|<p>New RAID level: {ITEM.LASTVALUE}</p>|information|
|RAID status require attention: {ITEM.LASTVALUE}|<p>Verifiy your device for more infos.</p>|high|
|Remote access status was changed for: {ITEM.LASTVALUE}|<p>-</p>|information|
|{HOST.NAME} has just been restarted|<p>-</p>|information|
|{#SNMPVALUE} status require attention: {ITEM.LASTVALUE}|<p>-</p>|high|
|Temperature of {#SNMPVALUE} is to high: {ITEM.LASTVALUE}|<p>-</p>|average|
|Temperature of {#SNMPVALUE} is to very high: {ITEM.LASTVALUE}|<p>-</p>|high|
|Operational status was changed on {HOST.NAME} interface {#SNMPVALUE}|<p>-</p>|information|
|Free disk space is less than 2% on volume {#SNMPVALUE}|<p>Used: {ITEM.LASTVALUE} Total: {ITEM.LASTVALUE2}</p>|high|
|Free disk space is less than 10% on volume {#SNMPVALUE}|<p>Used: {ITEM.LASTVALUE} Total: {ITEM.LASTVALUE2}</p>|warning|
