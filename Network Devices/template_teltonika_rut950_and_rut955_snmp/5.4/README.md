...
|Interface {#IFNAME}({#IFALIAS}): Inbound packets discarded (LLD)|<p>MIB: IF-MIB The number of inbound packets which were chosen to be discarded even though no errors had been detected to prevent their being deliverable to a higher-layer protocol. One possible reason for discarding such a packet could be to free up buffer space. Discontinuities in the value of this counter can occur at re-initialization of the management system, and at other times as indicated by the value of ifCounterDiscontinuityTime.</p>|`SNMP agent`|net.if.in.discards[ifInDiscards.{#SNMPINDEX}]<p>Update: 30m</p>|
|Interface {#IFNAME}({#IFALIAS}): Inbound packets with errors (LLD)|<p>MIB: IF-MIB For packet-oriented interfaces, the number of inbound packets that contained errors preventing them from being deliverable to a higher-layer protocol. For character-oriented or fixed-length interfaces, the number of inbound transmission units that contained errors preventing them from being deliverable to a higher-layer protocol. Discontinuities in the value of this counter can occur at re-initialization of the management system, and at other times as indicated by the value of ifCounterDiscontinuityTime.</p>|`SNMP agent`|net.if.in.errors[ifInErrors.{#SNMPINDEX}]<p>Update: 30m</p>|
|Interface {#IFNAME}({#IFALIAS}): Bits received (LLD)|<p>MIB: IF-MIB The total number of octets received on the interface, including framing characters. This object is a 64-bit version of ifInOctets. Discontinuities in the value of this counter can occur at re-initialization of the management system, and at other times as indicated by the value of ifCounterDiscontinuityTime.</p>|`SNMP agent`|net.if.in[ifHCInOctets.{#SNMPINDEX}]<p>Update: 3m</p>|
|Interface {#IFNAME}({#IFALIAS}): Outbound packets discarded (LLD)|<p>MIB: IF-MIB The number of outbound packets which were chosen to be discarded even though no errors had been detected to prevent their being deliverable to a higher-layer protocol. One possible reason for discarding such a packet could be to free up buffer space. Discontinuities in the value of this counter can occur at re-initialization of the management system, and at other times as indicated by the value of ifCounterDiscontinuityTime.</p>|`SNMP agent`|net.if.out.discards[ifOutDiscards.{#SNMPINDEX}]<p>Update: 30m</p>|
|Interface {#IFNAME}({#IFALIAS}): Outbound packets with errors (LLD)|<p>MIB: IF-MIB For packet-oriented interfaces, the number of outbound packets that contained errors preventing them from being deliverable to a higher-layer protocol. For character-oriented or fixed-length interfaces, the number of outbound transmission units that contained errors preventing them from being deliverable to a higher-layer protocol. Discontinuities in the value of this counter can occur at re-initialization of the management system, and at other times as indicated by the value of ifCounterDiscontinuityTime.</p>|`SNMP agent`|net.if.out.errors[ifOutErrors.{#SNMPINDEX}]<p>Update: 30m</p>|
...
|Interface {#IFNAME}({#IFALIAS}): Speed (LLD)|<p>MIB: IF-MIB An estimate of the interface's current bandwidth in units of 1,000,000 bits per second. If this object reports a value of `n' then the speed of the interface is somewhere in the range of `n-500,000' to`n+499,999'. For interfaces which do not vary in bandwidth or for those where no accurate estimation can be made, this object should contain the nominal bandwidth. For a sub-layer which has no concept of bandwidth, this object should be zero.</p>|`SNMP agent`|net.if.speed[ifHighSpeed.{#SNMPINDEX}]<p>Update: 30m</p>|
|Interface {#IFNAME}({#IFALIAS}): Operational status (LLD)|<p>MIB: IF-MIB The current operational state of the interface. - The testing(3) state indicates that no operational packet scan be passed - If ifAdminStatus is down(2) then ifOperStatus should be down(2) - If ifAdminStatus is changed to up(1) then ifOperStatus should change to up(1) if the interface is ready to transmit and receive network traffic - It should change todormant(5) if the interface is waiting for external actions (such as a serial line waiting for an incoming connection) - It should remain in the down(2) state if and only if there is a fault that prevents it from going to the up(1) state - It should remain in the notPresent(6) state if the interface has missing(typically, hardware) components.</p>|`SNMP agent`|net.if.status[ifOperStatus.{#SNMPINDEX}]<p>Update: 3m</p>|
|Interface {#IFNAME}({#IFALIAS}): Total daily traffic (LLD)|<p>Total traffic sent and received today. Resets to 0 at midnight each day. x bits/s / 8 = x bytes/s x bytes/s * 60 = x bytes/min x bytes/min * 6 = x bytes since last update 60*6/8=45</p>|`Calculated`|net.if.total[daily.{#SNMPINDEX}]<p>Update: 6m</p>|
|Interface {#IFNAME}({#IFALIAS}): Bits sent+received (LLD)|<p>MIB: IF-MIB The total number of octets received and sent on the interface. Will be 0 if host offline or rebooted to prevent "not supported" on calculation in total traffic.</p>|`Calculated`|net.if.total[ifbps.{#SNMPINDEX}]<p>Update: 1m</p>|
|Interface {#IFNAME}({#IFALIAS}): Total monthly traffic (LLD)|<p>Total traffic sent and received this month. Resets to 0 the first day every month.</p>|`Calculated`|net.if.total[monthly.{#SNMPINDEX}]<p>Update: 12m</p>|
|Interface {#IFNAME}({#IFALIAS}): Total weekly traffic (LLD)|<p>Total traffic sent and received this week. Resets to 0 every Monday.</p>|`Calculated`|net.if.total[weekly.{#SNMPINDEX}]<p>Update: 12m</p>|
...
|hssDwBandwidth for {#SNMPINDEX} (LLD)|<p>Max download bandwidth in b/s. Cannot be accessed using SNMPv1.</p>|`SNMP agent`|hssDwBandwidth[{#SNMPINDEX}]<p>Update: 300</p>|
|hssDwLimit for {#SNMPINDEX} (LLD)|<p>Download limit in bytes. Cannot be accessed using SNMPv1.</p>|`SNMP agent`|hssDwLimit[{#SNMPINDEX}]<p>Update: 300</p>|
|hssIdleTimeout for {#SNMPINDEX} (LLD)|<p>Idle timeout in sec.</p>|`SNMP agent`|hssIdleTimeout[{#SNMPINDEX}]<p>Update: 300</p>|
|hssID for {#SNMPINDEX} (LLD)|<p>Session ID</p>|`SNMP agent`|hssID[{#SNMPINDEX}]<p>Update: 300</p>|
|hssIndex for {#SNMPINDEX} (LLD)|<p>A unique value, greater than zero, for each session.</p>|`SNMP agent`|hssIndex[{#SNMPINDEX}]<p>Update: 300</p>|
|hssIP for {#SNMPINDEX} (LLD)|<p>Clients IP address</p>|`SNMP agent`|hssIP[{#SNMPINDEX}]<p>Update: 300</p>|
|hssMAC for {#SNMPINDEX} (LLD)|<p>Clients MAC address</p>|`SNMP agent`|hssMAC[{#SNMPINDEX}]<p>Update: 300</p>|
|hssState for {#SNMPINDEX} (LLD)|<p>Session state</p>|`SNMP agent`|hssState[{#SNMPINDEX}]<p>Update: 300</p>|
|hssTimeLimit for {#SNMPINDEX} (LLD)|<p>Time limit in sec. Cannot be accessed using SNMPv1.</p>|`SNMP agent`|hssTimeLimit[{#SNMPINDEX}]<p>Update: 300</p>|
|hssUpBandwidth for {#SNMPINDEX} (LLD)|<p>Max upwnload bandwidth in b/s. Cannot be accessed using SNMPv1.</p>|`SNMP agent`|hssUpBandwidth[{#SNMPINDEX}]<p>Update: 300</p>|
|hssUpLimit for {#SNMPINDEX} (LLD)|<p>Upload limit in bytes. Cannot be accessed using SNMPv1.</p>|`SNMP agent`|hssUpLimit[{#SNMPINDEX}]<p>Update: 300</p>|
|hssURL for {#SNMPINDEX} (LLD)|<p>Original URL</p>|`SNMP agent`|hssURL[{#SNMPINDEX}]<p>Update: 300</p>|
|hssUsername for {#SNMPINDEX} (LLD)|<p>Username</p>|`SNMP agent`|hssUsername[{#SNMPINDEX}]<p>Update: 300</p>|
|ICMP loss - operator {#OPERATOR} (LLD)|<p>-</p>|`Calculated`|icmppingloss-[{#OPERATORNUMBER}]<p>Update: 1m</p>|
|mdRevision for {#SNMPVALUE} (LLD)|<p>Modem revision</p>|`SNMP agent`|mRevision[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdTemperature for {#SNMPVALUE} (LLD)|<p>Modem temperature</p>|`SNMP agent`|mTemperature[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdSINR for {#SNMPVALUE} (LLD)|<p>>=20 Excellent. 20 to 13 Good. 13 to 11 Decent to slow, near 0 disconnects. <= 0 dB Unusable</p>|`SNMP agent`|mSINR[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdSimState for {#SNMPVALUE} (LLD)|<p>SIM status</p>|`SNMP agent`|mSimState[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdRSSI for {#SNMPVALUE} (LLD)|<p>> -65 Excellent. -64 to -75 Good. -76 to -85 Decent, may disconnect. -86 to -100 Slow with frequent disconnects.-101 to -125 Unusable. The average total received power observed only in OFDM symbols containing reference symbols for antenna port 0 (i.e., OFDM symbol 0 & 4 in a slot) in the measurement bandwidth over N resource blocks. The total received power of the carrier RSSI includes the power from co-channel serving & non-serving cells, adjacent channel interference, thermal noise, etc. Total measured over 12-subcarriers including RS from Serving Cell, Traffic in the Serving Cell</p>|`SNMP agent`|mSignal[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdRSSI for {#SNMPVALUE} operator {#OPERATOR} (LLD)|<p>> -65 Excellent. -64 to -75 Good. -76 to -85 Decent, may disconnect. -86 to -100 Slow with frequent disconnects.-101 to -125 Unusable. The average total received power observed only in OFDM symbols containing reference symbols for antenna port 0 (i.e., OFDM symbol 0 & 4 in a slot) in the measurement bandwidth over N resource blocks. The total received power of the carrier RSSI includes the power from co-channel serving & non-serving cells, adjacent channel interference, thermal noise, etc. Total measured over 12-subcarriers including RS from Serving Cell, Traffic in the Serving Cell</p>|`Calculated`|mSignal[{#SNMPVALUE}-{#OPERATORNUMBER}]<p>Update: 3m</p>|
|mdRSSI for {#SNMPVALUE} operator {#OPERATOR} cellID {#MCELLID} (LLD)|<p>> -65 Excellent. -64 to -75 Good. -76 to -85 Decent, may disconnect. -86 to -100 Slow with frequent disconnects.-101 to -125 Unusable. The average total received power observed only in OFDM symbols containing reference symbols for antenna port 0 (i.e., OFDM symbol 0 & 4 in a slot) in the measurement bandwidth over N resource blocks. The total received power of the carrier RSSI includes the power from co-channel serving & non-serving cells, adjacent channel interference, thermal noise, etc. Total measured over 12-subcarriers including RS from Serving Cell, Traffic in the Serving Cell</p>|`Calculated`|mSignal[{#SNMPVALUE}-{#MCELLID}]<p>Update: 3m</p>|
|mdSerial for {#SNMPVALUE} (LLD)|<p>Serial number</p>|`SNMP agent`|mSerial[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdRSRQ for {#SNMPVALUE} (LLD)|<p>(4G only) Quality considering also RSSI and the number of used Resource Blocks (N). RSRQ = (N * RSRP) / RSSI measured over the same bandwidth. RSRQ is a C/I type of measurement and it indicates the quality of the received reference signal. The RSRQ measurement provides additional information when RSRP is not sufficient to make a reliable handover or cell reselection decision. -3 to -10 Excellent. -11 to -15 Good. -16 to -19 Slow. –<-19.5 Unusable</p>|`SNMP agent`|mRSRQ[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdRSRP for {#SNMPVALUE} (LLD)|<p>(4G only) Reference Signal Receive Power is the average power of Resource Elements (RE) that carry cell specific Reference Signals (RS) over the entire bandwidth. RSRP does a better job of measuring signal power from a specific sector while potentially excluding noise and interference from other sectors. -44 to -80 Excellent. -81 to -109 Good. -110 to -120 Slow <-121 Unusable</p>|`SNMP agent`|mRSRP[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdRSRP for {#SNMPVALUE} operator {#OPERATOR} (LLD)|<p>(4G only) Reference Signal Receive Power is the average power of Resource Elements (RE) that carry cell specific Reference Signals (RS) over the entire bandwidth. RSRP does a better job of measuring signal power from a specific sector while potentially excluding noise and interference from other sectors. -44 to -80 Excellent. -81 to -109 Good. -110 to -120 Slow <-121 Unusable</p>|`Calculated`|mRSRP[{#SNMPVALUE}-{#OPERATORNUMBER}]<p>Update: 3m</p>|
|mdRSRP for {#SNMPVALUE} operator {#OPERATOR} cellID {#MCELLID} (LLD)|<p>(4G only) Reference Signal Receive Power is the average power of Resource Elements (RE) that carry cell specific Reference Signals (RS) over the entire bandwidth. RSRP does a better job of measuring signal power from a specific sector while potentially excluding noise and interference from other sectors. -44 to -80 Excellent. -81 to -109 Good. -110 to -120 Slow <-121 Unusable</p>|`Calculated`|mRSRP[{#SNMPVALUE}-{#MCELLID}]<p>Update: 3m</p>|
|mdPinState for {#SNMPVALUE} (LLD)|<p>PIN status</p>|`SNMP agent`|mPinState[{#SNMPVALUE}]<p>Update: 10m</p>|
|ICMP response time - operator {#OPERATOR} (LLD)|<p>-</p>|`Calculated`|icmppingsec-[{#OPERATORNUMBER}]<p>Update: 1m</p>|
|mdOperator for {#SNMPVALUE} (LLD)|<p>Operator currently in use</p>|`SNMP agent`|mOperator[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdOperatorNumber for {#SNMPVALUE} (LLD)|<p>Operator number (MCC+MNC)</p>|`SNMP agent`|mOperatorNumber[{#SNMPVALUE}]<p>Update: 3m</p>|
|mdNetState for {#SNMPVALUE} (LLD)|<p>Mobile network registration status</p>|`SNMP agent`|mNetState[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdModel for {#SNMPVALUE} (LLD)|<p>Modem model</p>|`SNMP agent`|mModel[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdManufacturer for {#SNMPVALUE} (LLD)|<p>Modem manufacturer</p>|`SNMP agent`|mManufacturer[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdIndex for {#SNMPVALUE} (LLD)|<p>A unique value, greater than zero, for each modem.</p>|`SNMP agent`|mIndex[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdIMSI for {#SNMPVALUE} (LLD)|<p>IMSI for simcard</p>|`SNMP agent`|mIMSI[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdIMEI for {#SNMPVALUE} (LLD)|<p>Modem IMEI</p>|`SNMP agent`|mImei[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdDescr for {#SNMPVALUE} (LLD)|<p>A textual string containing information about the modem.</p>|`SNMP agent`|mDescr[{#SNMPVALUE}]<p>Update: 60m</p>|
|mdConnectionType for {#SNMPVALUE} (LLD)|<p>Data session connection type</p>|`SNMP agent`|mConnectionType[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdConnectionState for {#SNMPVALUE} (LLD)|<p>Sata session connection state</p>|`SNMP agent`|mConnectionState[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdCellID for {#SNMPVALUE} (LLD)|<p>CELL ID</p>|`SNMP agent`|mCellID[{#SNMPVALUE}]<p>Update: 10m</p>|
|mdTraffic monthly for {#SNMPVALUE} operator {#OPERATOR} (LLD)|<p>Accumulated monthly mobile data usage for each operator Tested on RUT950 where wwan is always index 11 If your router has dual modems there will probably be multiple wwan interfaces so this item can be disabled.</p>|`Calculated`|mTraffic[{#SNMPVALUE}-{#OPERATORNUMBER}]<p>Update: 12m</p>|
|Network ARP entry {#IPPHYSADDRESS} (LLD)|<p>-</p>|`SNMP agent`|NetArpEntry[{#IPPHYSADDRESS}]<p>Update: 30m</p>|
|Network Interface #{#IFINDEX} IP (LLD)|<p>-</p>|`SNMP agent`|netInterfaceIP[{#IFINDEX}]<p>Update: 10m</p>|
|Network Interface #{#IFINDEX} Netmask (LLD)|<p>-</p>|`SNMP agent`|netInterfaceNetmask[{#IFINDEX}]<p>Update: 1d</p>|
|Storage {#FSNAME}: Space utilization (LLD)|<p>Space utilization in % for {#FSNAME}</p>|`Calculated`|vfs.fs.pused[storageUsedPercentage.{#SNMPINDEX}]<p>Update: 1d</p>|
|Storage {#FSNAME}: Total space (LLD)|<p>MIB: HOST-RESOURCES-MIB The size of the storage represented by this entry, in units of hrStorageAllocationUnits. This object is writable to allow remote configuration of the size of the storage area in those cases where such an operation makes sense and is possible on the underlying system. For example, the amount of main storage allocated to a buffer pool might be modified or the amount of disk space allocated to virtual storage might be modified.</p>|`SNMP agent`|vfs.fs.total[hrStorageSize.{#SNMPINDEX}]<p>Update: 1d</p>|
|Storage {#FSNAME}: Used space (LLD)|<p>MIB: HOST-RESOURCES-MIB The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits.</p>|`SNMP agent`|vfs.fs.used[hrStorageUsed.{#SNMPINDEX}]<p>Update: 1d</p>|
|{#MEMNAME}: Total memory (LLD)|<p>MIB: HOST-RESOURCES-MIB The size of the storage represented by this entry, in units of hrStorageAllocationUnits. This object is writable to allow remote configuration of the size of the storage area in those cases where such an operation makes sense and is possible on the underlying system. For example, the amount of main memory allocated to a buffer pool might be modified or the amount of disk space allocated to virtual memory might be modified.</p>|`SNMP agent`|vm.memory.total[hrStorageSize.{#SNMPINDEX}]<p>Update: 1d</p>|
|{#MEMNAME}: Used memory (LLD)|<p>MIB: HOST-RESOURCES-MIB The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits.</p>|`SNMP agent`|vm.memory.used[hrStorageUsed.{#SNMPINDEX}]<p>Update: 10m</p>|
|{#MEMNAME}: Memory utilization (LLD)|<p>Memory utilization in %</p>|`Calculated`|vm.memory.util[memoryUsedPercentage.{#SNMPINDEX}]<p>Update: 10m</p>|
## Triggers
